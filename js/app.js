...
const displayTeam = (teams) => {
    ...
    if (teams == null) {
        ...
        teams.forEach(team => {
            ...
            div.innerHTML = `
            <div class="card">
                <img src="${team.strTeamBadge}" class="img-fluid p-5" alt="..." />
                <div class="card-body">
                    <h5 class="card-title">${team.strTeam}</h5>
                    <p id="set-description" class="card-text">${team.strDescriptionEN?.slice(0, 150)}</p>
                </div>
...
const teamDetails = details => {
    console.log(details);
    document.getElementById('exampleModalLabel').innerText = `${details.strAlternate}`;
